...
_DECIMAL_PERCENT_RE = re.compile(r"(\d+(?:\.\d+)?)%")
...
# 连续 2~6 个字母，且必须至少包含一个大写
_ACRONYM_RE = re.compile(r"(?<![A-Za-z])([A-Za-z]{2,6})(?![A-Za-z])")

_SINGLE_LATIN_CJK = re.compile(r"(?<=[\u4e00-\u9fff])([A-Za-z])(?=[\u4e00-\u9fff])")
_MD_MARKERS_RE = re.compile(r"(^#+\s*|\*\*|__|`+)")
...
# NOTE:
# This normalizer is used by the XTTSv2 tokenizer. Downstream code may split text
# on commas to create chunks for TTS.
#
# If we translate *all* punctuation into commas, quotation marks such as “ ” / ‘ ’
# become commas as well, which can introduce artificial split points:
#   ，"电子爸妈"  ->  ,,电子爸妈,
# causing the sentence to be broken at the quoted phrase.
#
# To avoid that, we remove quote-like punctuation instead of turning it into
# commas, while keeping strong punctuation mapped to commas.
_QUOTE_LIKE_PUNCS = "\"'“”‘’„‟‹›«»＂＇"  # ASCII + common CN/Unicode quote marks

# replace punctuation with English comma, except quote-like punctuation which is removed
_NON_QUOTE_PUNCS = "".join(c for c in PUNCS if c not in _QUOTE_LIKE_PUNCS)

PUNCS_TRANSFORM = str.maketrans(
    _NON_QUOTE_PUNCS,
    "," * len(_NON_QUOTE_PUNCS),
)

_MULTI_COMMA_RE = re.compile(r",{2,}")
_EDGE_COMMA_RE = re.compile(r"^,+|,+$")
...
def time_to_zh(h: str, m: str) -> str:
    h = int(h)
    m = int(m)

    h_zh = number_to_zh(str(h))
    if m == 0:
        return f"{h_zh}点"
    else:
        return f"{h_zh}点{number_to_zh(str(m))}"


def normalize_time(text: str) -> str:
    # time range: 8:30-11:00
    def _range_repl(m):
    ...
    text = _TIME_RANGE_RE.sub(_range_repl, text)

    # single time: 8:30
    def _point_repl(m):
        h, m_ = m.groups()
        return time_to_zh(h, m_)

    text = _TIME_POINT_RE.sub(_point_repl, text)
    return text
...
KNOWN_ACRONYMS = {k.upper() for k in {
    "GDP", "CPI", "PMI", "AI", "CPU", "GPU", "API", "URL", "HTTP", "HTTPS"
}}


def expand_english_acronym(text):
    def _repl(m):
        word = m.group(1)
        u = word.upper()
        if u in KNOWN_ACRONYMS:
            return " ".join(u)
        return word
    return _ACRONYM_RE.sub(_repl, text)



def expand_single_latin(text):
    return _SINGLE_LATIN_CJK.sub(lambda m: m.group(1).upper(), text)
...
def number_to_zh(num_str: str) -> str:
    ...
    int_zh = int_to_zh(int_part)

    if frac_part:
        frac_zh = "".join(_DIGITS_ZH.get(ch, ch) for ch in frac_part if ch.isdigit())
        result = f"{sign}{int_zh}点{frac_zh}" if frac_zh else f"{sign}{int_zh}"
    else:
        result = f"{sign}{int_zh}"
    return result


def int_to_zh(int_str: str) -> str:
    """
    Convert non-negative integer string to Chinese (common spoken style).
    Examples:
      "0" -> "零"
      "5" -> "五"
      "10" -> "十"
    ...
    # For very large numbers, fall back to digit-by-digit reading (safe for TTS)
    # You can extend to 万/亿 if you want later.
    if L > 4:
        return "".join(_DIGITS_ZH[d] for d in digits)

    out = []
    zero_pending = False

    for i, d in enumerate(digits):
        pos = L - 1 - i  # 0=ones,1=tens,2=hundreds...
        if d == "0":
            zero_pending = True
            continue
        ...
        # handle 10-19: "十X" not "一十X"
        if pos == 1 and d == "1" and not out and L == 2:
            out.append("十")
        else:
            out.append(_DIGITS_ZH[d] + units[pos])
...
def normalize_percent(text):
    def repl(m):
        num = m.group(1)
        spoken = number_to_zh(num)   # must support decimals
        return f"百分之{spoken}"
...
#         # Be tolerant of formats like "5.5%" and "5.5 %".
#         num = re.sub(r"\s+", "", self.percentage)
#         num = num.strip().rstrip("%").strip()
#         return "百分之" + num2chn(num)
...
class Percentage:
    ...
    def percentage2chntext(self):
        # 支持 "5.5%"、"5.5 %"、" 5.5%" 等格式
        s = re.sub(r"\s+", "", self.percentage or "")
        s = s.strip()
        if not s.endswith("%"):
            return s
        num = s[:-1].strip()  # 去掉末尾的 %
        # 关键修改：统一使用 number_to_zh 处理整个数字（含小数）
        spoken_num = number_to_zh(num)
        return "百分之" + spoken_num
...
def normalize_percentages(text: str) -> str:
    def _repl(m: re.Match) -> str:
        raw = m.group(0)          # e.g. "5.5%" or "5.5 %"
        return Percentage(percentage=raw).percentage2chntext()
    return _PERCENT_RE.sub(_repl, text)
...
def normalize_nsw(raw_text):
    ...
    # # 规范化百分数
    # text = text.replace("％", "%")
    # # Allow optional whitespace before the percent sign: "5.5%" or "5.5 %".
...
class TextNorm:
    """
    Production-grade normalizer with explicit stage ordering.

    Design goals:
    - Stable, inspectable pipeline
    - No print() in hot path (use logging)
    ...
    def __init__(
        ...
        remove_erhua_flag: bool = False,
        check_chars: bool = False,
        remove_space_flag: bool = False,
        cc_mode: str = "",
        protect_quoted_spans: bool = True,
        add_comma_spacing: bool = True,
        debug: bool = False,
    ):
        if to_upper and to_lower:
            raise ValueError("Only one of to_upper / to_lower can be True.")

        self.to_banjiao = to_banjiao
        self.to_upper = to_upper
        self.to_lower = to_lower
        self.remove_fillers = remove_fillers
        self.remove_erhua_flag = remove_erhua_flag
        self.check_chars = check_chars
        self.remove_space_flag = remove_space_flag
        self.protect_quoted_spans = protect_quoted_spans
        self.add_comma_spacing = add_comma_spacing
        self.debug = debug
    ...
    def _build_pipeline(self) -> List[Stage]:
        stages: List[Stage] = []

        # 0) Basic whitespace normalization (cheap + helps regex stability)
        def _normalize_ws(t: str) -> str:
            t = t.replace("\\n", "\n")   # ← 新增
            t = t.replace("\u00A0", " ")
            return t.strip()

        stages.append(Stage("ws_normalize", _normalize_ws))

        # 1) OpenCC conversion
        if self.cc:
            stages.append(Stage("opencc_convert", lambda t: self.cc.convert(t)))

        # 2) fullwidth -> halfwidth
        if self.to_banjiao:
            stages.append(Stage("qj2bj", lambda t: t.translate(QJ2BJ_TRANSFORM)))

        # 3) case normalize
        if self.to_upper:
            stages.append(Stage("to_upper", lambda t: t.upper()))
        if self.to_lower:
            stages.append(Stage("to_lower", lambda t: t.lower()))
        ...
        if self.remove_fillers:
            def _remove_fillers(t: str) -> str:
                for c in FILLER_CHARS:
                    t = t.replace(c, "")
                return t
            stages.append(Stage("remove_fillers", _remove_fillers))

        # 5) erhua
        if self.remove_erhua_flag:
            stages.append(Stage("remove_erhua", remove_erhua))

        stages.append(Stage("strip_markdown", strip_markdown))

        # 6
        stages.append(Stage("expand_single_latin", expand_single_latin))
        stages.append(Stage("expand_english_acronym", expand_english_acronym))

        # 7
        stages.append(Stage("normalize_nsw", normalize_nsw))


        # 8-11) Quote protect + punctuation mapping + restore
        # Put these stages as a single stage to avoid re.compile every call.
        stages.append(Stage("punctuation_chunking", self._punctuation_chunking_stage))

        # 12) illegal chars
        if self.check_chars:
            stages.append(Stage("check_chars", self._check_chars_stage))

        # 13) remove_space (english word spacing policy)
        if self.remove_space_flag:
            stages.append(Stage("remove_space", remove_space))

        return stages

    def _punctuation_chunking_stage(self, text: str) -> str:
        lines = text.split("\n")
        lines = [self._punctuation_chunking_line(l) for l in lines]
        return "\n".join(lines)

    def _punctuation_chunking_line(self, text: str) -> str:
        spans = []
        S0 = "\uE000"
        S1 = "\uE001"

        if self.protect_quoted_spans:
            def _protect(m):
                idx = len(spans)
                spans.append(m.group(1))
                return f"{S0}{idx}{S1}"
            text = _QUOTED_SPAN_RE.sub(_protect, text)

        text = text.translate(str.maketrans("", "", _QUOTE_LIKE_PUNCS))
        text = text.translate(PUNCS_TRANSFORM)
        text = _MULTI_COMMA_RE.sub(",", text)
        text = _EDGE_COMMA_RE.sub("", text)

        if self.add_comma_spacing:
            text = _COMMA_SPACING_RE.sub(" , ", text)
            text = _MULTI_SPACE_RE.sub(" ", text).strip()

        if self.protect_quoted_spans and spans:
            import re as _re
            _pat = _re.compile(_re.escape(S0) + r"(\d+)" + _re.escape(S1))
            def _restore(m):
                return spans[int(m.group(1))]
            text = _pat.sub(_restore, text)

        return text


    def _check_chars_stage(self, text: str) -> str:
        for c in text:
            if not IN_VALID_CHARS.get(c):
                # Return empty to skip line, but do not print
                logger.warning("Illegal char %r in text: %r", c, text)
                continue
        return text
    ...
    def __call__(self, text: str) -> str:
        ...
        for st in self._stages:
            old = t
            t = st.fn(t)
            if self.debug and old != t:
                logger.debug("Stage %s changed text: %r -> %r", st.name, old, t)
            if not t:
                return ""
        return t
...
if __name__ == "__main__":
    ...
    p.add_argument("ifile", default= "/data/fu.dai/code/Auralis-Enhanced/examples/input.txt", help="input filename, assume utf-8 encoding")
    p.add_argument("ofile", default="/data/fu.dai/code/Auralis-Enhanced/examples/input_result.txt", help="output filename")
    ...
    normalizer = TextNorm(
        ...
        remove_erhua_flag=args.remove_erhua,
        check_chars=args.check_chars,
        remove_space_flag=args.remove_space,
